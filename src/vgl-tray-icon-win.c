...
notify_playback_thread (VglTrayIconPlaybackData *d)
{
        ...
        if (priv->now_playing) {
				...
				if (priv->show_notifications) {
					/*Cafrada mode: on*/
					GtkStatusIconPrivate *tray_priv;
					tray_priv = priv->tray_icon->priv;

					NOTIFYICONDATA icon_data = {0};
					icon_data = tray_priv->nid;
					icon_data.cbSize = sizeof(NOTIFYICONDATA);
					icon_data.uFlags = NIF_INFO;
					icon_data.dwInfoFlags = NIIF_NONE;
					/*szInfoTitle member is declared as TCHAR[64]  */
					g_strlcpy(icon_data.szInfoTitle, APP_NAME,64);
					gchar *message = g_strconcat(g_strndup(tooltip_string,255), NULL);
					/*szInfo member is declared as TCHAR[256]  */
					g_strlcpy(icon_data.szInfo, message, 256);
					icon_data.uTimeout = 10000; // in milliseconds
...
vgl_tray_icon_notify_playback (VglTrayIcon *vti, LastfmTrack *track)
{
        ...
        gdk_threads_add_idle((GSourceFunc)notify_playback_thread,data);
}
